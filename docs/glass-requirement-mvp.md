...
**在稳定、可持续运行的前提下，持续向手机回传 JPEG 图流；用户唤醒后，进入连续对话模式并持续回传图片与音频，直到连续静音超阈值后再回落 Idle，供手机完成一轮或多轮实时图分析。**

### 2.1 成功标准

- Idle 档可持续推图，连续运行 10 分钟无明显掉流、重启、内存泄漏
- 唤醒后会话可完成一次连续对话交互：升档图流 + 连续音频推流 + 静音退出 + 正常回落 Idle
...
```text
眼镜持续采图 → 用户唤醒 → 眼镜升档进入连续对话并持续传图+音 → 手机 STT/选帧/MiniCPM/TTS
```
...
| `SESSION_ACTIVE` | VGA 2.5fps 持续推 JPEG + 16kHz PCM；连续静音、超时或强制结束后发 `SESSION_END` |

升档流程：**先停采 → 修改分辨率/质量 → 再开采**。允许约 0.3–0.5s 图流间隙，手机端用唤醒前缓存帧兜底。进入 `SESSION_ACTIVE` 后，图像档位保持高档，直到会话真正退出才降回 Idle。
...
- 处理：原始 PDM 音频需先做基础过滤，再进行能量检测
- 退出规则：连续静音超过阈值后自动结束当前连续对话会话
- 配置方式：静音退出阈值、音频最长时长、会话总超时均通过配置项定义，**统一以秒配置**
- 默认值：
  - 静音退出阈值：`3s`
  - 音频最长时长：分钟级，可配置
  - 会话总超时：分钟级，可配置
...
- 眼镜端只负责「开门」与「关门」：发 `CONTROL_WAKE` 并升档进入连续对话；在静音退出或超时后发 `CONTROL_SESSION_END`
...
2. PDM 麦克风采集、基础过滤与 Active 连续对话音频推流
3. WiFi STA 联网与 TCP Server
4. 二进制帧协议封包与发送
5. `IDLE_STREAM` ↔ `SESSION_ACTIVE` 连续对话状态机
6. 板载按键 + 串口命令触发唤醒/结束
...
| 配置 | 关键参数（WiFi SSID/密码、端口、帧率、静音退出秒数、音频上限秒数、会话超时秒数）可通过头文件或编译宏调整 |
...
3. 按键或串口 `w` 后：收到 `CONTROL_WAKE` → 持续升档 `IMAGE` + `AUDIO`
4. 连续静音达到阈值、超时或串口 `e` 后：收到 `CONTROL_SESSION_END`，并自动回落 Idle 档
...
| 功耗发热 | Active 连续对话期间持续高档采图 + WiFi + 音频 | Idle 压至 1fps；限制最长会话时长；实机观察温升 |
...
**Idle 档 WiFi 持续推 JPEG → 唤醒后升档进入连续对话并持续推 JPEG + PCM → 连续静音超阈值或超时后回落 Idle**
